...
use crate::backend::models::{GroupKey, ServiceError, TileGroup, GROUP_DIM_I32};
...
pub async fn test_cache() {
    ...
    set_cached_tile_group(conn, &group1).await.unwrap();
    ...
    set_cached_tile_group(conn, &group2).await.unwrap();

    let group1 = get_cached_tile_group(conn, GroupKey(0, 0)).await.unwrap();
    let group2 = get_cached_tile_group(conn, GroupKey(GROUP_DIM_I32, GROUP_DIM_I32)).await.unwrap();
    let group3 = get_cached_tile_group(conn, GroupKey(GROUP_DIM_I32 * 2, GROUP_DIM_I32 * 2)).await.unwrap();
